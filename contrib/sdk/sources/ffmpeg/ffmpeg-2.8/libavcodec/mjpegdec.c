            ...
            if(!mb_x || mb_y == resync_mb_y || mb_y == resync_mb_y+1 && mb_x < resync_mb_x || s->interlaced){
                ...
                for (i = 0; i < nb_components; i++) {
                    ...
                    for(j=0; j<n; j++) {
                        ...
                        ptr = s->picture_ptr->data[c] + (linesize * (v * mb_y + y)) + (h * mb_x + x); //FIXME optimize this junk
                        if(y==0 && toprow){
                            if(x==0 && leftcol){
                                pred= 1 << (bits - 1);
                            }else{
                        ...
                        *ptr= pred + (dc << point_transform);
                            ...
                            ptr16 = (uint16_t*)(s->picture_ptr->data[c] + 2*(linesize * (v * mb_y + y)) + 2*(h * mb_x + x)); //FIXME optimize this junk
                            if(y==0 && toprow){
                                if(x==0 && leftcol){
                                    pred= 1 << (bits - 1);
                                }else{
                    ...
                    for (j = 0; j < n; j++) {
                        ...
                        if (   h * mb_x + x >= s->width
                            ...
                            ptr = s->picture_ptr->data[c] +
                              (linesize * (v * mb_y + y)) +
                              (h * mb_x + x); //FIXME optimize this junk
                            PREDICT(pred, ptr[-linesize-1], ptr[-linesize], ptr[-1], predictor);
